...
class Prompt:
    ...
    def _derive_Eiso_jet(self, params, theta_c_rad):
        ...
        if "chi_1" not in params:
            raise KeyError("BH spin missing: needed for risco -> disk mass -> E_iso_jet")
        chi_1 = float(params["chi_1"])
        ...
        mb = float(calculate_baryonic_mass(m2, C2))
        risco = float(calculate_r_isco(chi_1))
        m_disk = float(calculate_bhns_disk_mass_foucart(C2, eta, risco, mb, params["m_dyn_jet"]))  # assumed Msun
